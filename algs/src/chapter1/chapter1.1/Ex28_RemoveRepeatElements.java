...
public class Ex28_RemoveRepeatElements {

    // sorted array detection
    public static boolean detectArray(int[] arr) {
        for (int i = 0; i < arr.length; i++) {
            int idx = Ex28_RemoveRepeatElements.indexof(arr, arr[i], 0, arr.length - 1);
            if (idx != -1 && idx != i)
                return false;
        }
        return true;
    ...
    public static void main(String[] args) {
        ...
        // detetion result
        if (Ex28_RemoveRepeatElements
                .detectArray(Ex28_RemoveRepeatElements.removeRepeatElements(whitelist)))
