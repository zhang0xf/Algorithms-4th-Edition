...
// BinarySearch find Equivalent（相等的） key
...
public class Ex29_EquivalentKey {

    // return array elements(<key) nums
    public static int rank(int key, int[] arr) {
        int idx = Ex29_EquivalentKey.indexOf(arr, key);
        if (idx != -1) {
            while (idx > 0) {
                if (arr[idx] >= key)
                    idx--;
    ...
    public static int count(int key, int[] arr) {
        ...
        int count = 0;
        int idx = -1;
        while ((idx = Ex29_EquivalentKey.indexOf(tempArray, key)) != -1) {
            tempArray[idx] = key - 1;
            count++;
        }
        return count;
